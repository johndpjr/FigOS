...
void* process_malloc(struct process* process, size_t size)
{
    ...
    // TODO: when there are multiple tasks per process, map to each task of the process
...
void process_free(struct process* process, void* ptr)
{
    ...
    // TODO: unmap the page entry (security flaw)
    kfree(ptr);
}
...
{
    for (int i = 0; i < PEACHOS_MAX_PROGRAM_ALLOCATIONS; ++i)
        process_free(process, process->allocations[i].ptr); // TODO: can be more efficient (just kfree)
