...
void isr80h_register_command(int command_id, ISR80H_COMMAND command)
{
    if (command_id < 0 || command_id >= PEACHOS_MAX_ISR80H_COMMANDS) {
        // Kernel shouldn't setup command incorrectly, panic
        panic("isr80h_register_command(): command is out of bounds\n");
    }

    if (isr80h_commands[command_id]) {
        // Only one command can bind
        panic("isr80h_register_command(): command is already registered\n");
    }
